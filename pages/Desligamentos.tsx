...
import { 
  ...
  FileText, UserCheck, BarChart, TrendingDown, ArrowLeft
} from 'lucide-react';
...
export const Desligamentos: React.FC = () => {
  ...
  const openInterviewForm = (emp: Employee) => {
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in">
      ...
      {/* ======================= VISÃO DE FORMULÁRIO (SUBPÁGINA) ======================= */}
      {view === 'form' && interviewingEmp && (
        <div className="max-w-5xl mx-auto animate-in slide-in-from-right-4 duration-300">
          
          <button onClick={() => { setView('list'); setInterviewingEmp(null); }} className="flex items-center gap-2 text-slate-500 hover:text-rose-600 font-bold mb-6 transition-colors group">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 flex flex-col">
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50 rounded-t-3xl">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-sm text-slate-600 font-medium">
                <span className="flex items-center gap-1 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm"><UserMinus size={16} className="text-slate-400"/> {interviewingEmp.name}</span>
                <span className="flex items-center gap-1 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm"><Briefcase size={16} className="text-slate-400"/> {interviewingEmp.role} ({interviewingEmp.sector})</span>
                <span className="flex items-center gap-1 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg shadow-sm text-amber-800"><Calendar size={16} className="text-amber-500"/> Adm: {formatDateBR(interviewingEmp.admissionDate)}</span>
                <span className="flex items-center gap-1 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg shadow-sm text-red-800"><Calendar size={16} className="text-red-500"/> Demissão: {formatDateBR((interviewingEmp as any).terminationDate)}</span>
              </div>
            </div>
            ...
            <div className="p-8">
              {formData.reason === 'Não Aplicável (Dispensado/Demitido)' ? (
                <div className="text-center py-16">
                   <div className="w-20 h-20 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-4"><CheckSquare size={40} /></div>
                   <h3 className="text-2xl font-bold text-slate-800">Entrevista Não Aplicável</h3>
                   <p className="text-slate-500 mt-2 text-lg">O colaborador foi demitido ou dispensado da entrevista. Ela não conta nas métricas.</p>
                </div>
              ) : formData.didNotRespond ? (
                <div className="text-center py-16">
                   <div className="w-20 h-20 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4"><X size={40} /></div>
                   <h3 className="text-2xl font-bold text-slate-800">Colaborador optou por não responder</h3>
                   <p className="text-slate-500 mt-2 text-lg">Esta entrevista foi arquivada sem dados coletados, mas conta como recusa no relatório.</p>
                </div>
              ) : (
                <form id="exit-form" onSubmit={(e) => handleSave(e, 'NORMAL')} className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-rose-50/50 p-6 rounded-2xl border border-rose-100">
                    <div className="space-y-2">
                      ...
                      <input type="date" required className="w-full border border-rose-200 p-3.5 rounded-xl outline-none focus:ring-2 focus:ring-rose-500 bg-white font-bold" value={formData.interviewDate} onChange={e => setFormData({...formData, interviewDate: e.target.value})} />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        required 
                        className="w-full border border-rose-200 p-3.5 rounded-xl outline-none focus:ring-2 focus:ring-rose-500 bg-white font-bold" 
                    ...
                    {isOtherReason && (
                      <div className="space-y-2 md:col-span-2 animate-in fade-in">
                        ...
                        <input type="text" required className="w-full border border-rose-200 p-3.5 rounded-xl outline-none focus:ring-2 focus:ring-rose-500 bg-white" placeholder="Escreva o motivo..." value={formData.reason} onChange={e => setFormData({...formData, reason: e.target.value})} />
                      </div>
                    )}
                    ...
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <textarea rows={3} className="w-full border border-rose-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-rose-500 bg-white resize-none" placeholder="Detalhes adicionais sobre o desligamento..." value={formData.reasonObservation} onChange={e => setFormData({...formData, reasonObservation: e.target.value})}></textarea>
                    </div>
                  </div>

                  {/* PESQUISA DE CLIMA (RATINGS) */}
                  <div>
                    <h3 className="text-lg font-black text-slate-800 border-b border-slate-200 pb-3 mb-6 uppercase tracking-tighter">Avaliação da Experiência na Empresa</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-100 pt-8">
                    <div className="space-y-5 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                      <h4 className="font-bold text-slate-700 flex items-center gap-2 text-lg"><UserCheck size={20} className="text-indigo-500"/> Gestão e Liderança</h4>
                      <div>
                        <label className="text-xs font-bold text-slate-500 block mb-2">Nome do Líder Direto</label>
                        <input type="text" list="employee-names" className="w-full border border-slate-300 p-3 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-sm" placeholder="Quem era o gestor?" value={formData.leaderName} onChange={e => setFormData({...formData, leaderName: e.target.value})} />
                      </div>
                      <RatingSelector label="Relação com o Líder" value={formData.leaderRating!} onChange={v => setFormData({...formData, leaderRating: v})} />
                    </div>

                    <div className="space-y-5 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                      <h4 className="font-bold text-slate-700 flex items-center gap-2 text-lg"><Briefcase size={20} className="text-emerald-500"/> Treinamento Recebido</h4>
                      <div>
                        <label className="text-xs font-bold text-slate-500 block mb-2">Quem realizou o treinamento?</label>
                        <input type="text" list="employee-names" className="w-full border border-slate-300 p-3 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 bg-white text-sm" placeholder="Nome do instrutor/colega" value={formData.trainerName} onChange={e => setFormData({...formData, trainerName: e.target.value})} />
                      </div>
            ...
            <div className="p-6 border-t border-slate-200 bg-slate-50 rounded-b-3xl flex flex-col sm:flex-row items-center justify-between shrink-0 gap-4">
               <div className="flex gap-3 w-full sm:w-auto">
                  {(!formData.didNotRespond && formData.reason !== 'Não Aplicável (Dispensado/Demitido)') && (
                     <>
                       <button type="button" onClick={(e) => handleSave(e, 'DID_NOT_RESPOND')} className="flex-1 sm:flex-none text-xs font-bold text-rose-600 bg-rose-100 hover:bg-rose-200 px-4 py-3 rounded-xl transition-colors">
                          Recusou Responder
                       </button>
                       <button type="button" onClick={(e) => handleSave(e, 'NOT_APPLICABLE')} className="flex-1 sm:flex-none text-xs font-bold text-slate-600 bg-slate-200 hover:bg-slate-300 px-4 py-3 rounded-xl transition-colors">
               ...
               <div className="flex gap-3 w-full sm:w-auto">
                 <button type="button" onClick={() => { setView('list'); setInterviewingEmp(null); }} className="flex-1 sm:flex-none px-6 py-3 font-bold text-slate-500 hover:bg-slate-200 rounded-xl transition-colors">Cancelar</button>
                 {(!formData.didNotRespond && formData.reason !== 'Não Aplicável (Dispensado/Demitido)') && (
                   <button 
                     type="submit" form="exit-form"
                     className="flex-1 sm:flex-none bg-rose-600 hover:bg-rose-700 text-white px-8 py-3 rounded-xl font-bold shadow-lg transition-all active:scale-95 disabled:opacity-50"
