...
export const GeneralInterviews: React.FC = () => {
  // O "as any" aqui blinda o código contra erros chatos da Vercel caso addTalent seja novo no contexto
  const { candidates, addCandidate, updateCandidate, removeCandidate, jobs, verifyUserPassword, addTalent } = useData() as any;
  ...
  const [redirectJobId, setRedirectJobId] = useState(''); 
  ...
  const allGeneralCandidates = useMemo(() => {
    return (candidates || [])
        .filter((c: Candidate) => c.jobId === GENERAL_POOL_ID)
        .sort((a: Candidate, b: Candidate) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }, [candidates]);

  const filteredList = useMemo(() => {
    return allGeneralCandidates.filter((c: Candidate) => {
        const matchesSearch = c.name.toLowerCase().includes(searchTerm.toLowerCase());
        const isArchivedStatus = ['Reprovado', 'Desistência', 'Reentrevistado'].includes(c.status);
  ...
  const pendingFeedbackCandidates = useMemo(() => {
      return allGeneralCandidates.filter((c: Candidate) => 
          c.interviewAt && 
          !['Reprovado', 'Desistência', 'Contratado', 'Reentrevistado'].includes(c.status)
      ).sort((a: Candidate, b: Candidate) => new Date(a.interviewAt!).getTime() - new Date(b.interviewAt!).getTime());
  }, [allGeneralCandidates]);

  const openJobs = useMemo(() => (jobs || []).filter((j: any) => j.status === 'Aberta'), [jobs]);
  ...
  const handleOpenModal = (candidate?: Candidate) => {
    if (candidate) {
        ...
        if (candidate.status === 'Reentrevistado') {
            setRedirectJobId(candidate.rejectionReason || '');
            setLossReasonType('');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!['Reprovado', 'Desistência', 'Reentrevistado'].includes(payload.status || '')) {
        ...
        if (payload.status === 'Reentrevistado') {
            payload.rejectionReason = redirectJobId;
        }
    }

    if (editingId) {
        const original = candidates.find((c: Candidate) => c.id === editingId);
      ...
      {pendingFeedbackCandidates.length > 0 && (
          <section className="bg-amber-50 border border-amber-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pendingFeedbackCandidates.map((c: Candidate) => {
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                      {filteredList.map((c: Candidate) => (
                          <tr key={c.id} className={`hover:bg-slate-50 transition-colors ${['Reprovado', 'Desistência', 'Reentrevistado'].includes(c.status) ? 'opacity-70 bg-slate-50/50' : ''}`}>
                              <td className="p-4">
                              ...
                              <td className="p-4">
                                  <div className="flex items-center gap-2">
                                      {c.phone}
                                      {getWaLink(c.phone) && <a href={getWaLink(c.phone)!} target="_blank" rel="noreferrer" className="text-green-600 hover:scale-110 transition-transform"><MessageCircle size={16}/></a>}
                                  </div>
                              ...
                              <td className="p-4">
                                  <span className={`px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap
                                      ${c.status === 'Reprovado' ? 'bg-red-100 text-red-700' : 
                                        ...
                                        c.status === 'Reentrevistado' ? 'bg-purple-100 text-purple-700' : 
                                        'bg-blue-50 text-blue-700'}`}>
                                      {c.status}
                                  </span>
                                  {c.status === 'Reentrevistado' && c.rejectionReason && (
                                      <div className="text-[10px] text-purple-600 font-medium mt-1 truncate max-w-[150px]" title={jobs.find((j:any) => j.id === c.rejectionReason)?.title || c.rejectionReason}>
                                          ↳ {jobs.find((j:any) => j.id === c.rejectionReason)?.title || c.rejectionReason}
                                      </div>
                                  )}
                              </td>
                              <td className="p-4 text-right flex justify-end gap-2">
                                  ...
                                  <button onClick={() => handleOpenLink(c)} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded" title="Transferir para uma Vaga"><ExternalLink size={16}/></button>
                                  ...
                                  <button onClick={() => handleDeleteClick(c)} className="p-2 text-red-600 hover:bg-red-50 rounded" title="Excluir Duplicado"><Trash2 size={16}/></button>
                              </td>
                          </tr>
                      ))}
                  </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
           <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md animate-fadeIn">
              ...
              <p className="text-sm text-center text-slate-500 mb-6">
                  Isso irá <strong>MOVER</strong> o(a) <strong>{candidateToLink.name}</strong> para a vaga selecionada e o removerá da Entrevista Geral.
              </p>
              ...
              <div className="flex gap-2">
                  ...
                  <button onClick={confirmLink} disabled={!selectedJobId} className="flex-1 bg-indigo-600 text-white font-bold py-3 rounded-xl hover:bg-indigo-700 shadow-lg disabled:opacity-50">Confirmar Transferência</button>
              </div>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                  ...
                  <button onClick={() => setIsModalOpen(false)} className="hover:bg-slate-100 p-1 rounded-full"><span className="text-slate-400 font-bold">X</span></button>
              </div>
                      ...
                      <div><label className="block text-xs font-bold mb-1">Status</label>
                          <select 
                            ...
                            onChange={e => {
                                ...
                                setFormData({...formData, status: newStatus as EmployeeStatus});
                                if (!['Reprovado', 'Desistência', 'Reentrevistado'].includes(newStatus)) {
                                    setLossReasonType(''); 
                                    setRedirectJobId('');
                          ...
                          >
                              ...
                              <option value="Reentrevistado">Reentrevistado (Direcionado)</option>
                          </select>
                      </div>
                  </div>

                  {/* CAMPO DE REENTREVISTADO */}
                  {formData.status === 'Reentrevistado' && (
                      <div className="bg-purple-50 p-4 rounded-lg border border-purple-200 animate-fadeIn">
                          <label className="block text-xs font-bold text-purple-700 mb-1 flex items-center gap-1">
                              <Briefcase size={14}/> Para qual vaga ele foi direcionado? (Obrigatório)
                          </label>
                          <select className="w-full border border-purple-300 p-2 rounded bg-white text-sm" 
                              value={redirectJobId} 
                              onChange={e => setRedirectJobId(e.target.value)}
                              required
                          >
                              <option value="">-- Selecione a Vaga --</option>
                              {jobs.map((j: any) => <option key={j.id} value={j.id}>{j.title} ({j.unit})</option>)}
                          </select>
                          <p className="text-xs text-purple-600 mt-2">Isso arquiva a Entrevista Geral como "Concluída", permitindo que você avalie o candidato na vaga específica sem duplicar as métricas de pendências.</p>
                      </div>
                  )}
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-red-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
           <div className="bg-white rounded-xl p-6 w-full max-w-sm border-t-4 border-red-500 shadow-2xl">
              ...
              <p className="text-xs text-slate-500 mb-4">Digite sua senha para confirmar a exclusão permanente de {candidateToDelete?.name}. Útil para remover candidatos duplicados.</p>
                 ...
                 <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button onClick={() => { 
                        if (addTalent) {
                            ...
                            alert('O salvamento para banco não está habilitado no momento.');
                        }
